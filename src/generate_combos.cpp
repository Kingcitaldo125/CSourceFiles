...
void combos(std::vector<std::vector<int>>& coins, set_t& xset, const std::vector<int>& reference, std::vector<int>& current, const std::size_t magnitude, const int idx)
{
    if (idx >= static_cast<int>(reference.size()))
    {
        const auto& c_size = current.size();

        if (c_size == 0 || c_size != magnitude) return;

        std::sort(current.begin(), current.end());

        auto cur_str = stringify(current);
        //std::cout << "cur_str: " << cur_str << "\n";
        if (xset.find(cur_str) != xset.end()) return;
...
void generate_combos_help(std::vector<std::vector<int>>& t_coins, std::vector<int> holder, set_t& xset, const std::size_t magnitude)
{
