...
template <class T> inline std::vector<T> catvec(const T &first, const std::vector<T> &second)
{
    std::vector<T> out{first};
    std::copy_if(second.begin(), second.end(), std::back_inserter(out),
                 [&first](const T &t) { return t != first; });

    return out;
}

void perm_help(std::vector<std::vector<int>> &outmatx, std::vector<int> &input, const int isize,
               const int idx)
{
    if (isize < 2)
        return;
...
std::vector<std::vector<int>> perm(const int N)
{
    ...
    {
        std::sort(outmatx.begin(), outmatx.end(),
